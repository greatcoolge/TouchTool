...
object DisplayUtils {
    ...
    /**
     * 获取屏幕宽度（显示宽度，横屏的时候可能会小于物理像素值） 根据当前方向返回像素
     */
    fun getScreenWidth(context: Context): Int {
        val metrics = DisplayMetrics()
        val manager = context.getSystemService(Context.WINDOW_SERVICE) as WindowManager
        manager.defaultDisplay.getRealMetrics(metrics)
        return if (manager.defaultDisplay.rotation % 2 == 0) {
            if (metrics.widthPixels > metrics.heightPixels) metrics.heightPixels else metrics.widthPixels
        } else {
            val width = if (metrics.widthPixels > metrics.heightPixels) metrics.widthPixels else metrics.heightPixels
            width - getNavigationBarCurrentHeight(context)
        }
    }

    /**
     * 获取屏幕高度（物理像素值的高度） 根据当前方向返回像素
     */
    ...
    fun rejectedNavHeight(context: Context): Int {
        ...
        val manager = context.getSystemService(Context.WINDOW_SERVICE) as WindowManager
        if (manager.defaultDisplay.rotation % 2 == 0){
            return screenHeight - getNavigationBarCurrentHeight(context)
        }
        return screenHeight
    }
